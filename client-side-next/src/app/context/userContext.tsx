...
const UserContext = createContext({id:'', username:'', email:''});

export function UserWrapper({children} : {
    children: React.ReactNode;
}) {
    const [userState, setUserState] = useState({id:'', username:'', email:''})

    useEffect(() => {
        const getUserInfo = async () => {
            try{
              const userData = await axios.get('/api/users/info')
              setUserState({id:userData.data.message.id, username:userData.data.message.username, email:userData.data.message.email})
            }catch(err:any){
